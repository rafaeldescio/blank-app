...
from utils import get_blobclient
...
def load_data() -> pd.DataFrame:
    ...
    blob_client = get_blobclient()
    # Download the blob content as bytes
    downloaded_blob_data = blob_client.download_blob().readall()

    # Read the bytes into a Pandas DataFrame
    return pd.read_parquet(BytesIO(downloaded_blob_data), engine='pyarrow') # or 'fastparquet'
